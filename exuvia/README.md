...
Estructura: `js/simulator.js` (fuentes simuladas WHOOP / Apple Health / registro manual) → `js/ingest.js` (normalizador) → `js/genome.js` (rasgos y reglas) → `js/mutations.js` (mudas y exuvias) → `js/organism.js` (shader de partículas). `js/share.js` exporta imagen, animación y enlace. `js/main.js` arma la interfaz.
