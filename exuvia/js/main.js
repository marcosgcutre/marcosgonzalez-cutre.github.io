...
function featureBars(f) {
  return Object.entries(FEATURES).map(([k, m]) => `
    <div class="row"><span>${m.label} <span class="tag ${m.scale}">${m.scale}</span></span>
      <div class="track"><b style="width:${(f[k] * 100).toFixed(0)}%"></b></div><span>${(f[k] * 100).toFixed(0)}</span></div>`).join('');
}

function renderLab(rebuild = true) {
  if (!S.labFeatures) S.labFeatures = { ...entry().features };
...
$('#lab-on').onchange = (e) => {
  ...
  if (S.lab) S.labFeatures = S.labFeatures ?? { ...entry().features };
  refresh();
};
...
function renderShare() {
  const ind = indicators(S.days, S.idx);
...
const shareInfo = () => {
  const e = entry();
  return {
    mutantId: String(PERSONAS[S.persona].seed).padStart(6, '0'), stage: e.stage, name: e.name ?? '', age: e.age,
    indicators: indicators(S.days, S.idx), seed: 4721, time: performance.now() / 1000,
  };
};
...
$('#share-img').onclick = async () => {
  ...
  try { const r = await shareImage(stage.organism.genome, shareInfo(), { include: shareInclude }); status(r === 'shared' ? 'Compartido.' : r === 'cancelled' ? '' : 'Imagen descargada.'); }
  catch (e) { status(e.message); } finally { busy(false); }
};
$('#share-vid').onclick = async () => {
  busy(true);
  try {
    const r = await shareVideo(stage.organism.genome, shareInfo(), { include: shareInclude }, 4, (p) => status(`Grabando ${Math.round(p * 100)}%`));
    status(r === 'shared' ? 'Compartido.' : r === 'cancelled' ? '' : 'Animación descargada.');
  } catch (e) { status(e.message); } finally { busy(false); }
};
$('#share-link').onclick = async () => {
  const i = shareInfo();
  const url = encodeGenome(stage.organism.genome, { stage: i.stage, name: i.name, mutantId: i.mutantId });
...
let lastStage = null;
$('#scrub').oninput = (e) => { S.idx = +e.target.value; S.ghost = null; $('#ghost-label').hidden = true; announceMutation(); refresh(); };
